...
export default function SurveyReport() {
  ...
  return (
    <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur flex items-center justify-center fade-in">
      <div className="w-full max-w-2xl bg-[#121820] rounded-2xl p-8 border border-[#1a202c] shadow-2xl">
        <div className="text-center mb-8">
          <div className="inline-block bg-green-500/20 text-green-500 p-4 rounded-full mb-4">
            <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
          </div>
          <h2 className="text-3xl font-bold text-white tracking-wide">SURVEY COMPLETE</h2>
        </div>

        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="bg-[#171f2a] p-4 rounded-xl border border-[#1a202c]">
            <div className="text-xs text-gray-500 uppercase font-mono mb-1">Distance</div>
            <div className="text-2xl text-white font-mono">{formatDistance(distance)}</div>
          </div>
          <div className="bg-[#171f2a] p-4 rounded-xl border border-[#1a202c]">
          ...
          <div className="bg-[#171f2a] p-4 rounded-xl border border-[#1a202c]">
            ...
            <div className="text-2xl text-white font-mono">{samples}</div>
          </div>
          <div className="bg-[#171f2a] p-4 rounded-xl border border-[#1a202c]">
            <div className="text-xs text-gray-500 uppercase font-mono mb-1">Anomalies</div>
            <div className="text-2xl text-red-400 font-mono font-bold">{anomalyCount}</div>
          </div>
          <div className="bg-[#171f2a] p-4 rounded-xl border border-[#1a202c]">
            <div className="text-xs text-gray-500 uppercase font-mono mb-1">Cavities</div>
            <div className="text-2xl text-orange-400 font-mono">{cavities}</div>
          </div>
          <div className="bg-[#171f2a] p-4 rounded-xl border border-[#1a202c]">
            <div className="text-xs text-gray-500 uppercase font-mono mb-1">Leaks</div>
            <div className="text-2xl text-blue-400 font-mono">{leaks}</div>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="h-4 w-full rounded-full overflow-hidden flex bg-gray-800">
            <div style={{ width: `${pCalm}%` }} className="bg-green-500" title={`Calm: ${pCalm.toFixed(1)}%`} />
            <div style={{ width: `${pWatch}%` }} className="bg-yellow-500" title={`Watch: ${pWatch.toFixed(1)}%`} />
            <div style={{ width: `${pInspect}%` }} className="bg-red-500" title={`Inspect: ${pInspect.toFixed(1)}%`} />
          </div>
        ...
        {anomalies.length > 0 && (
          <div className="mb-8">
            ...
            <div className="space-y-2 max-h-32 overflow-y-auto">
              {anomalies.map((a, i) => (
                <div key={a.id || i} className="flex justify-between items-center bg-[#171f2a] p-2 rounded border border-[#1a202c] text-xs font-mono">
                  <span className={a.type === 'cavity' ? 'text-orange-400' : 'text-blue-400'}>{a.type.toUpperCase()}</span>
        ...
        <div className="flex justify-center gap-4">
          <button onClick={handleReset} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold uppercase tracking-wider transition">RESET SURVEY</button>
        </div>
      </div>
    </div>
  );
