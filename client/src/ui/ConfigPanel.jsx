...
export default function ConfigPanel() {
  ...
  return (
    <div className="absolute right-0 top-12 bottom-0 w-80 bg-[#0e1116] border-l border-[#151920] p-5 z-40 overflow-y-auto slide-in shadow-2xl">
      ...
      <div className="space-y-6">
        <div>
          ...
          <input type="range" min="0.1" max="3.0" step="0.1" value={config.sensorNoise} onChange={(e) => handleChange('sensorNoise', parseFloat(e.target.value))} className="w-full accent-[#52657a]" />
        </div>
        ...
        <div>
          ...
          <input type="range" min="0.5" max="3.0" step="0.1" value={config.detectionThreshold} onChange={(e) => handleChange('detectionThreshold', parseFloat(e.target.value))} className="w-full accent-[#52657a]" />
        </div>
        ...
        <div>
          ...
          <input type="range" min="5" max="30" step="1" value={config.baselineWindow} onChange={(e) => handleChange('baselineWindow', parseInt(e.target.value))} className="w-full accent-[#52657a]" />
        </div>
        ...
        <div>
          ...
          <input type="range" min="2" max="15" step="1" value={config.roverMaxSpeed} onChange={(e) => handleChange('roverMaxSpeed', parseInt(e.target.value))} className="w-full accent-[#52657a]" />
        </div>
        ...
        <div className="space-y-3">
          {[
            ...
            <label key={key} className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" checked={config[key]} onChange={(e) => handleChange(key, e.target.checked)} className="accent-[#52657a] w-4 h-4 rounded bg-gray-800 border-gray-600" />
